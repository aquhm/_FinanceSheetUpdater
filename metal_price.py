...
# def get_commodity_prices(commodities, period='30d'):
#     try:
...
#             # NaN 값 제거 및 유효한 데이터만 포함
#             valid_data = resampled.dropna()

#             # Timestamp를 문자열로 변환하고 NaN이 아닌 값만 포함
#             data[name] = {date.strftime('%Y-%m-%d'): float(price) 
#                           for date, price in valid_data.items() 
#                           if not np.isnan(price)}
        
#         return data
#     except Exception as e:
#         logging.error(f"Error in get_commodity_prices for period {period}: {str(e)}")
#         return None
...
#         try:
#             data = get_commodity_prices(commodities, period)
#             if data is not None:
#                 all_data[period] = data
#                 print(f"Successfully fetched data for period: {period}")
#         except Exception as e:
#             logging.error(f"Error processing period {period}: {str(e)}")
#             print(f"An error occurred while processing period {period}. Check the log file for details.")
